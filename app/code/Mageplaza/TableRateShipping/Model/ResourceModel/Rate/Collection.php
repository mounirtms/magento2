...
class Collection extends AbstractCollection
{
    ...
    public function filterByRequest($request, $cartData)
    {
        ...
        /* filter shipping group */
        $shippingGroups = $this->getShippingGroup($request);

        $condition = 'shipping_group IS NULL or shipping_group = ""';
        foreach ($shippingGroups as $shippingGroup) {
            $condition .= ' or FIND_IN_SET(' . $shippingGroup . ',shipping_group)';
        }
