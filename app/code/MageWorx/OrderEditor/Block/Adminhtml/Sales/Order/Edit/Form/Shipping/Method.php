...
class Method extends ShippingMethodForm
{
    ...
    public function getShippingRates(): array
    {
        if (empty($this->_rates)) {
            ...
            $address = $this
                ->getAddress()
                ->setQuote($this->getQuote()->setQuoteCurrencyCode($this->getOrder()->getOrderCurrencyCode()))
            ...
            $this->_eventManager->dispatch(
                'mageworx_order_editor_adminhtml_block_get_shipping_rates_after',
                [
                    'rates'                => $this->_rates,
                    'order'                => $this->getOrder(),
                    'shipping_method_form' => $this
                ]
            );
        }
     ...
     * @return string
     */
    public function getChildrenBlockHtml($rate, string $code = ''): string
    {
        $html = '';

        if (!is_a($rate, \Magento\Framework\DataObject::class)) {
            return $html;
        }
        ...
        try {
            /**
             * In this method using $this->getLayout(), where throw LocalizedException
             */
            $childBlockNameList = is_array($this->getChildNames()) ? $this->getChildNames() : [];
        } catch (\Magento\Framework\Exception\LocalizedException $exception) {
            return $html;
        }

        foreach ($childBlockNameList as $pos => $childBlockName) {
            try {
                $html .= $this->getChildBlock($childBlockName)
                              ->setData('rate', $rate)
                              ->setData('code', $code)
                              ->setData('pos', $pos + 1)
                              ->toHtml();
            } catch (\Magento\Framework\Exception\LocalizedException $exception) {
                $this->_logger->warning('An error occurred while trying to render a block with the name ' . $childBlockName);
                continue;
            }
        }
