...
class Quote
{
    ...
    protected function prepareProductOptions(
        ...
        if (empty($productOptions) || empty($productOptions['info_buyRequest'])) {
            return $params;
        }

        $optionsFromOrderItem = $productOptions['info_buyRequest']['options'];

        if ($optionsFromOrderItem && is_array($optionsFromOrderItem)) {
            foreach ($optionsFromOrderItem as $optionId => $value) {
                $optionActionKey = 'options_' . $optionId . '_file_action';
                if (isset($params[$optionActionKey]) && $params[$optionActionKey] == 'save_old') {
                    $params['options'][$optionId] = $value;
                    unset($params[$optionActionKey]);
                }
            }
        }
